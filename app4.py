...
def detect_plate(image_path):
    ...
    # 🔍 Cek apakah plat nomor sudah ada di Firestore
    last_entry = (
        db.collection("detected_plates")
        .where("plate_number", "==", text)
    ...
    # Hitung durasi sejak terakhir kali plat ini terbaca
    duration = "New entry"
    if last_timestamp:
        last_time = datetime.strptime(last_timestamp, "%Y-%m-%d %H:%M:%S %Z")
        time_diff = local_now - last_time
        duration = f"{time_diff.total_seconds() // 3600} hour(s)"

    # 🔥 Simpan hasil ke Firebase Firestore
    data = {
        "plate_number": text,
        "timestamp": timestamp,
        "duration": duration,  # ⏳ Durasi pembacaan terakhir
